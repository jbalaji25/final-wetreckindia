...
interface AddTrekkingFormProps {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    initialData?: any;
    isEditing?: boolean;
}

const AddTrekkingForm: React.FC<AddTrekkingFormProps> = ({ initialData, isEditing = false }) => {
    ...
    const [itinerary, setItinerary] = useState([{ day: 1, title: '', description: '', altitude: '', distance: '' }]);
    ...
    const addItineraryDay = () => {
        setItinerary(prev => [...prev, { day: prev.length + 1, title: '', description: '', altitude: '', distance: '' }]);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (image) {
                ...
                const imageResponse = await fetch(`${API_URL}/api/trekking/upload`, {
                    method: 'POST',
                    body: imageFormData
                });
            ...
            if (gallery.length > 0) {
                ...
                const galleryResponse = await fetch(`${API_URL}/api/trekking/upload-multiple`, {
                    method: 'POST',
            ...
            // 3. Create or Update Trekking Package
            const trekkingData = {
                ...formData,
                image: imageUrl,
                gallery: galleryUrls,
            ...
            const url = isEditing
                ? `${API_URL}/api/trekking/${initialData._id}`
                : `${API_URL}/api/trekking`;
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify(trekkingData)
            });

            const data = await response.json();

            if (data.success) {
                alert(`Trekking package ${isEditing ? 'updated' : 'added'} successfully!`);

                if (!isEditing) {
                    // Reset form
                    setFormData({
                        title: '', location: '', difficulty: 'Moderate', rating: 0, description: '', detailedDescription: '',
                        bestTime: '', difficulty_details: '', trekFee: '', reportingDates: '', contact: '', trekLevel: '',
                        trekDuration: '', highestAltitude: '', suitableFor: '', totalTrekDistance: '', basecamp: '',
                        accommodationType: '', pickupPoint: ''
                    });
                    setImage(null);
                ...
                alert(`Failed to ${isEditing ? 'update' : 'add'} trekking package: ` + data.message);
            }
        } catch (error) {
            console.error('Error adding/updating trekking package:', error);
            alert('Error adding/updating trekking package');
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-md p-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">{isEditing ? 'Edit Trekking Package' : 'Add New Trekking Package'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {/* Trek Details */}
                <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-gray-700 border-b pb-2">Trek Details</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Trek Fee</label>
                            <input type="text" name="trekFee" value={formData.trekFee} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Duration (e.g., 7 days)</label>
                            <input type="text" name="trekDuration" value={formData.trekDuration} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Highest Altitude</label>
                            <input type="text" name="highestAltitude" value={formData.highestAltitude} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Total Distance</label>
                            <input type="text" name="totalTrekDistance" value={formData.totalTrekDistance} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Basecamp</label>
                            <input type="text" name="basecamp" value={formData.basecamp} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Best Time</label>
                            <input type="text" name="bestTime" value={formData.bestTime} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Suitable For (Age/Experience)</label>
                            <input type="text" name="suitableFor" value={formData.suitableFor} onChange={handleInputChange} placeholder="e.g., Ages 18-50, Beginners welcome" className="w-full px-4 py-2 border rounded-lg" />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    {/* Inclusions */}
                    <div>
                        <h3 className="text-lg font-semibold text-gray-700 border-b pb-2 mb-4">Inclusions</h3>
                        {inclusions.map((item, index) => (
                            <div key={index} className="flex gap-2 mb-2">
                                <input
                                    type="text"
                                    value={item}
                                    onChange={(e) => handleListChange(index, e.target.value, setInclusions)}
                                    placeholder="Enter inclusion"
                                    className="flex-1 px-4 py-2 border rounded-lg"
                                />
                                <button
                                    type="button"
                                    onClick={() => removeListItem(index, setInclusions)}
                                    className="p-2 text-red-500 hover:bg-red-50 rounded-lg"
                                >
                                    <Trash2 className="w-5 h-5" />
                                </button>
                    ...
                    {/* Exclusions */}
                    <div>
                        <h3 className="text-lg font-semibold text-gray-700 border-b pb-2 mb-4">Exclusions</h3>
                        {exclusions.map((item, index) => (
                            <div key={index} className="flex gap-2 mb-2">
                                <input
                                    type="text"
                                    value={item}
                                    onChange={(e) => handleListChange(index, e.target.value, setExclusions)}
                                    placeholder="Enter exclusion"
                                    className="flex-1 px-4 py-2 border rounded-lg"
                                />
                                <button
                                    type="button"
                                    onClick={() => removeListItem(index, setExclusions)}
                                    className="p-2 text-red-500 hover:bg-red-50 rounded-lg"
                                >
                                    <Trash2 className="w-5 h-5" />
                                </button>
                ...
                <div className="pt-6 border-t">
                    ...
                    >
                        {isSubmitting ? (isEditing ? 'Updating Trek...' : 'Adding Trek...') : (isEditing ? 'Update Trekking Package' : 'Add Trekking Package')}
                    </button>
                </div>
            </form>
        </div>
