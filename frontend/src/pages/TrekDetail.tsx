...
const TrekDetail = () => {
  ...
  useEffect(() => {
    const fetchTrek = async () => {
      try {
        const response = await fetch(`${API_URL}/api/trekking/${id}`);
        const data = await response.json();
        if (data.success) {
          const trek = data.trek;
          trek.image = trek.image.replace('http://localhost:5002', API_URL);
          if (trek.gallery && trek.gallery.length > 0) {
            trek.gallery = trek.gallery.map((img: string) => img.replace('http://localhost:5002', API_URL));
          }
          setPackageData(trek);
        } else {
  ...
  if (error || !packageData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">{error || 'Trek not found'}</h2>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {packageData.highlights && packageData.highlights.map((highlight: string, index: number) => (
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {packageData.itinerary && packageData.itinerary.map((day: { day: number; title: string; description: string; altitude?: string; distance?: string }, index: number) => (
            ...
            <div className="mb-8">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <ul className="space-y-2">
                  {packageData.preparation && packageData.preparation.map((item: string, index: number) => (
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {packageData.gallery && packageData.gallery.map((image: string, index: number) => (
          ...
          <div className="lg:col-span-1">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg sticky top-24">
              ...
              <div className="mb-6">
                ...
                <ul className="space-y-2">
                  {packageData.inclusions && packageData.inclusions.map((item: string, index: number) => (
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {packageData.exclusions && packageData.exclusions.map((item: string, index: number) => (
                    <li key={index} className="flex items-start text-sm">
